...
const CreateAcademicFaculty = () => {
  ...
  const onSubmit:SubmitHandler<FieldValues> = async(data)=>{
    ...
    try {
      ...
      if(res?.data) toast.success("Academic faculty added successfully", {id:toastId})
      if(res?.error) toast.error((res?.error?.data?.errorSource?.[0]?.message ||res?.error?.data?.message)  , {id:toastId})
        console.log(res)
    } catch (error:any) {
  ...
  return (
    <Flex justify="center">
      <Col span={8}>
        <UMSForm resolver={zodResolver(academicFacultySchema)}  onSubmit={onSubmit}>
